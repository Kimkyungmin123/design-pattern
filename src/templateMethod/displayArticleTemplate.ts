...
export default abstract class DisPlayArticleTemplate {
  ...
  // readonly, arrow function 을 사용해서 파생 클래스에서는  displayString메서드를 재정의 할 수 없도록 함.
  // 이렇게 파생 클래스에서 코드를 변경하지 못하도록 막으면, 좀 더 코드가 견고해지지만,
  // 이를 지원하지 않는 언어에서는 굳이 이렇게 하지 않아도 됨.

  public readonly displayString = () => {
    return [this.titleString(), this.contentString(), this.footerString()];
  };

  // titleString, contentString, footerString 매서드를 구현이 없는 추상 매서드로 선언
  protected abstract titleString(): string;
  protected abstract contentString(): string;
  protected abstract footerString(): string;
}
